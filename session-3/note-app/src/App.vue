...
const updateNotes = () => {
  isEditModalOpne.value = true;
  const updatedNoteIndex = notes.value.findIndex(note => note.id === updatedID.value);
  notes.value[updatedNoteIndex].text = newNote.value;
  updatedID.value = null;
  newNote.value = "";
  isEditModalOpne.value = false;
}
  ...
  <main>
    <div v-if="isModalOpne" class="overlay">
      <div class="modal">
        ...
        <button class="close" @click="isModalOpne = false, newNote = ''">Close</button>
      </div>
    </div>
    <div v-if="isEditModalOpne" class="overlay">
      <div class="modal">
        <textarea v-model.trim="newNote" name="note" id="note" cols="30" rows="10"></textarea>
        <p class="error" v-if="errorMessage">{{ errorMessage }}</p>
        <button @click="() => updateNotes()">Edit Note</button>
        <button class="close" @click="isEditModalOpne = false, newNote = ''">Close</button>
      </div>
    </div>
    <div class="container">
      <header>
